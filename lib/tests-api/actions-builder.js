...
module.exports = class ActionsBuilder {
    ...
    switchToFrame(frameElement) {
        this._pushAction(this.switchToFrame, function switchToFrame(browser) {
            return findElement(frameElement, browser).then(browser._wd.frame);
        });
    ...
    switchToWindow() {
        this._pushAction(this.switchToWindow, function switchToWindow(browser) {
            return browser._wd.window('');
        });
